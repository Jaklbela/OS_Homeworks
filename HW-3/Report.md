...
```
## Оценка 6-7.
## К основной программе добавляется клиент-наблюдатель, который подключаетися к серверу и получает все данные от него для их последующего вывода. Код клиента здесь тот же.
